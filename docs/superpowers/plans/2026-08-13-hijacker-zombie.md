...
3. 实现 1000 基础生命、普通速度、50 啃咬与 `NORMAL/LOCKED/FINALIZING/RESOLVED` 状态。
...
4. 修改 `AddNightRoofCharge`/`UpdateNightRoofCharge`：75% 只锁定一次；100% 选择 7 秒或既有 4 秒；最后 1 秒通知实体；死亡/掉头只取消处决，不改变普通放电计时。
