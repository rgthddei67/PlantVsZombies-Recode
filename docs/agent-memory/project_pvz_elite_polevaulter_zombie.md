...
撑杆阻拦路径在弃杆进入 `WALKING` 后调用植物反馈，再 `StartEat`，最后调用可覆写的
`OnVaultBlocked`。精英覆写该钩子：先在同排同 X 创建普通撑杆，再通过
`TakeBodyDamage(800)` 结算绕过僵尸减伤与免伤次数的固定本体伤害。当前基础生命 450，伤后生命钳到 0；
必须等待后续逻辑帧再断言 `anim_death`，同一碰撞帧仍可能显示 `anim_eat`。召唤先于伤害，
所以即使致死也仍满足“生成普通撑杆”契约。

`smoke_tallnut_elite_pole.json` 叠满僵尸减伤并赋予 8 次免伤，断言阻拦后仍为生命 0、
免伤次数仍为 8、`anim_death`、弃杆状态和普通撑杆存在；截图确认高坚果、垂死精英与召唤体同屏。
