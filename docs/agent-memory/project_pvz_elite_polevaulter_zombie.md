...
`TakeDamage(800, DamageSource::ZOMBIE)`；800 的承受者是高坚果，不是精英本体。普通关高坚果
从 8000 降至 7200，精英保持 450 并进入 `anim_eat`。该伤害走标准僵尸对植物链，生存模式的
僵尸增伤和植物韧性继续统一生效。

`smoke_tallnut_elite_pole.json` 先断言精英处于 `JUMPING/anim_jump`、高坚果仍 8000、Bonk 为 0；
越过 60% 后断言精英仍 450、正在啃食，高坚果为 7200、实际跳距和额外补偿均归零，并存在召唤的普通撑杆。
