...
- 预警转放电的唯一边沿会对锁定行形成一次稳定实体 ID 快照并结算基础玩法；绝缘僵尸通过通用提供者/目标虚接口加入承接与湿坡改写，劫持者在同一边沿先做全场处决快照，避雷花盆通过 under 层语义接口加入同格保护和同排增伤；台风改向仍未实现。
...
- 普通瓦面上，锁定行所有活动非花盆植物停机 8 游戏秒；地面僵尸承受 75 点 `OTHER` 环境伤害，非车辆再麻痹 0.75 游戏秒。
- 若锁定行在放电瞬间正处于径流 `FLOWING`，仅坡面区升级为强导电：植物停机 20 秒，僵尸承受 120 伤害并麻痹 1.2 秒；同一行平台仍使用普通数值。花盆不停机，飞行与地下阶段连伤害都免疫，冰车/投篮车等车辆只受伤不麻痹。
...
- 绝缘者若放电瞬间正处于湿坡，固定只扣陶瓷甲 360 且不溢出本体，并沿用 1.2 秒麻痹；离坡后的
  6 秒残留湿润只改变植物对甲伤害与抗性，不把平台放电升级为湿坡固定伤害。
- 避雷花盆有效承载上层时，同格上层免本次离散停机；锁定行若有至少一只有效承载，本次合格地面
  僵尸伤害取 2 倍（75/120→150/240），麻痹不变，多盆取最大值。资格只在放电边沿按固定列查询，
...
- `GameInfoSaver` 保存 `nightRoofCharge/Overcharge/Phase/PhaseTimer/Row`、劫持者选择已尝试/实体 ID/预警延长/终局标志/本波计数、植物 `shutdownTimer` 与僵尸保护字段 `paralysisTimer`。旧档缺字段使用中性值；活动阶段快照往返继续原行、余时、余电和既有劫持资格，不重新挑选。
