...
- 预警转放电的唯一边沿会对锁定行形成一次稳定实体 ID 快照并结算基础玩法；绝缘僵尸已通过通用提供者/目标虚接口加入承接与湿坡改写，处决僵尸劫持、避雷植物和台风改向仍未实现。
...
- 避雷植物、台风改向和处决僵尸劫持仍属第六大关延期内容；除已定案的绝缘僵尸交互外，基础放电不得预先实现其他反制或改写已确认数值。
- 第六大关目前已有绝缘僵尸专属出怪编排，但仍没有专属植物奖励与 6-9 BOSS 运行逻辑。
