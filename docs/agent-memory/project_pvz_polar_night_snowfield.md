...
`docs/superpowers/specs/2026-08-28-polar-night-snowfield-design.md`。8-1～8-9 的植物、僵尸和出怪表现已在
后续迭代完成；2026-09-04 又加入通关第八大关后解锁的极夜无尽 1007。新增内容必须继续复用这里的
背景、环境资格和正式波次创建边界，不能复制第二套雪原状态；第八大关只是当前登记边界，不是最终区域。
...
极夜无尽从第一轮直接运行完整导演，不继承上述教学关保证，也不会触发 8-9 的 60 秒特殊终波；
其生存轮次、词条和 `level1007` 存档仍走通用无尽契约。
...
2026-09-05 修正雪穴延迟出生与提前出波血量采样的时序：原实现只统计已创建实体，五路全部改道时
本波血量和阈值均为 0；事务清空到下一次 0.5 秒采样之间，会误判清波。修复前可见 ×1 隔离场景
在无植物、无伤亡条件下 4 秒从第 1 波跳到第 3 波。现在兑现边沿立即刷新指标，
`FinalizeWaveSpawnThreshold()` 等本波候选全部兑现后才按完整当前血量锁定原有 50%～65% 阈值；
无延迟的波次仍在生成后立即锁定。待出生列表本身承载读档中的未完成状态，无需新增存档字段；
旧波次事务只刷新指标，不重抽当前波阈值。专项为 `smoke_snow_hole_wave_pacing.json`。
本次 `clang-release` 构建及 378 项 Win7 导入审计通过；可见专项分批执行 143 + 25 条命令，覆盖
×1/×4/×9、部分改道、预警读档、封穴回退和无洞对照，均 exit 0；完整血量 540 时保持第 1 波，
杀掉一只降至 270 后正常进入第 2 波。原 `smoke_polar_night_core` 84 条命令也 exit 0，状态、日志、
七场景与原核心截图已核对。adding-zombie 技能及出怪编排 reference 已审计，补充计时契约并通过校验。
