...
## 2026-08-23：粒子 Field 完整冻结

- 暂停时仍会执行用于 UI 和输入的零 `deltaTime` 逻辑步。此前 `ParticleEmitter::Update`
  虽未推进寿命和物理，却仍会让 `Shake` 每步重新抽样，并让逐步语义的 `Friction` 继续
  衰减速度，因此大喷菇、忧郁菇和毁灭菇等粒子会在暂停画面中抖动或改变恢复后的轨迹。
- 发射器现在在缩放后的游戏时间不推进时完整保留上一游戏帧状态；绘制仍照常执行，恢复
  后从原状态继续。`Shake` 不重抽、`Friction` 不衰减，其他发射、寿命、动画和 Field
  状态也不在暂停期间变化。
- `smoke_particle_pause.json` 用毁灭菇 23 颗粒子和忧郁菇八向云 40 颗粒子分别暂停 45 个
  渲染帧。`clang-release` 构建与 Win7 378 项导入审计通过；当前桌面可见运行 54 条命令、
  退出码 0、日志以 `script finished OK` 结束且无 FAIL/ERROR/WARN。两组暂停前后状态均
  完全相同，两组同步 PNG 的 SHA-256 也分别完全一致；修复前同一毁灭菇用例的两张 PNG
  哈希不同，证明专项能检出原问题。
