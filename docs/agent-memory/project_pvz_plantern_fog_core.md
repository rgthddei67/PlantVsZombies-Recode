...
- `PLANT_PLANTERN` 在冒险 4-1 作为奖励，25 阳光、30 秒冷却；4-1 只保留雾视觉，4-2～4-9
  及固定复用完整迷雾的 6-9 启用燃料、照明、产光增益和雾中远程索敌限制。
...
- 生存词条 `PLANT_MIST_REFINING` 仅在启用完整路灯花机制的地图进入候选池；每层同时把死亡时
  实际领取的单份雾火与 `Plantern::ReserveFuel` 的本波并发接收上限提高 50%，最多两层。
  它不改变 100 点仓储容量、基础供给曲线或携带者分配。对应僵尸词条 `ZOMBIE_FOG_BREAKOUT`
  也只在同类地图出现，以逐格雾 alpha 阈值判断首次“被雾遮挡→可见”边沿并清控免控 3 秒。
...
- 2026-08-02 图鉴卡 host 重查修复完成 `clang-release` 配置/构建；
  `smoke_plant_almanac_card_host` 在当前桌面可见运行 exit 0，从主菜单真实点击进入植物图鉴后
  目标警告为 0，再进入 4-2 断言路灯花 III 挡并目验卡牌挡位/燃料截图。
- 2026-08-15 `smoke_level_6_9_fog` 在当前桌面可见运行 98 条命令 exit 0：6-9
...
- 2026-08-27 `smoke_survival_perk_fog_and_toxin` 在默认与 `-NoInstance` 均通过；同靶对照锁定
  无词条单份 10/本波上限 45，两层精炼为 20/90，并验证浓雾中的麻痹僵尸被 III 挡照亮后
  触发一次突围、解除麻痹且获得剩余约 1.5 秒的控制免疫。截图已目验照明与雾边。
