...
2026-08-02 完成，2026-08-04 调整为 100 阳光、30 秒冷却、4000 生命。`PumpkinShell : Plant`
...
- 跳跃阻拦不复用战斗顶层：`Board::GetJumpBlockingPlantAt` 仍按外到内询问能力，但不会阻拦的南瓜
  会继续向内找到高坚果；受阻后的普通啃食仍回到南瓜顶层，保持外壳优先承伤。
...
- 图鉴文案明确说明九宫格保护下精英小丑与粉色橄榄球使用 `×5`、爆破工头使用 `×4`，
  并注明普通小丑仍会直接摧毁爆区植物，避免玩家把所有爆炸误认为都受外壳保护。
- `Board::ApplyPumpkinProtectedZombieAreaDamage` 是特殊僵尸范围扣血的统一入口：先沿各技能
  原几何收集命中植物，再为每个命中层选择自身逻辑九宫格内最近的活动南瓜；距离相同按
  行、列、实体 ID 稳定打破并列，同一爆炸按保护南瓜 ID 归并一次。南瓜只为自己承伤，
  不由相邻南瓜继续转移；没有保护者时维持各实际命中层独立承伤。默认输入 `×5`，精英
  矿工爆破通过显式重载使用独立 `×4`。当前接入精英小丑投盒、精英矿工爆破和粉色橄榄球
  掉盔；普通小丑的直接 `Die()` 爆炸明确不接入。精英小丑贪心与蒙特卡洛快照保存南瓜层，
  并使用相同的九宫格选择和保护者归并规则。
...
- 2026-08-04 九宫格保护完成 `clang-release` 构建，增量复核 exit 0。当前桌面可见
  `smoke_pumpkin_zombie_area_damage` 72 条命令 exit 0：斜向精英小丑由南瓜承受
  `50×5=250`、九宫格外核桃仍扣 50；爆破工头的多命中与重叠保护稳定归并到一枚南瓜，
  只扣 `150×4=600`，未被选中的重叠南瓜保持 4000，圈外核桃仍扣 150；水池相邻行南瓜
  一次保护 under+normal，普通小丑继续清空整组。4 张同步截图已逐张检查。
- 同日新增 `smoke_pumpkin_zombie_area_targeting`，当前桌面可见 47 条命令 exit 0；关闭
  蒙特卡洛的贪心与启用 32 rollout 的蒙特卡洛均识别九宫格保护，把目标从受保护的低血
  向日葵改投未保护的低血三线射手。两张同步截图与 `run.log` 已检查。
- 同批按当前 100/30/4000 数值重跑 `smoke_pumpkin`，主人当前桌面可见 115 条命令 exit 0；
  资源、正反种植顺序、顶层啃食、两档破损、外壳死亡、存档、水池及三层台风均通过，
  6 张同步截图已逐张检查。
