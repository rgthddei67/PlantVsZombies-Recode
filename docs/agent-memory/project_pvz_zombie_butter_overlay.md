...
- follower 的状态 overlay 默认开启，装备会继承宿主的减速/冻结等着色；黄油在 `ConfigureButterSplatFollower()` 中显式传 `inheritOverlayEffect=false`。两条绘制路径都把每个 follower 的 overlay 紧跟在其 base 后提交，避免延迟 follower 的着色层跨槽覆盖黄色黄油。
