...
- Animator 的 follower 保存 `drawAfterAllTracks` 策略。2026-08-22 起 follower 参数与附加子 Animator 不再常驻每个 `TrackExtraInfo`，只为实际使用的轨道建立按索引排序的 `SparseTrackState`；轨道可见性、自定义纹理和偏移仍留在连续热数组。普通品种默认把已计算好的 follower `InstanceRecord` 延迟到本 Animator 全部轨道与子附件之后提交，使黄油位于头发、眼镜等最高层；巨人覆写为 `false`，紧随 `anim_head1` 提交，让后续木杆、下巴和前臂继续自然遮挡。
