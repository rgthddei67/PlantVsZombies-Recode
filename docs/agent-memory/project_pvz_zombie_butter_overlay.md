...
- follower 的状态 overlay 默认开启，装备会继承宿主的减速/冻结着色；属于当前承伤层外观的装备可显式开启父轨 additive glow，冰像黑盔使用该入口同步实际承伤白光。黄油在 `ConfigureButterSplatFollower()` 中显式同时关闭 `inheritOverlayEffect` 与 `inheritGlowEffect`。实例与 CPU batch 两条路径都按每个 follower 的 base、overlay、glow 顺序紧邻提交，避免延迟 follower 的效果层跨槽覆盖黄色黄油。
...
- 2026-08-26 命名 vector follower 与 overlay 分流改造后，`smoke_zombie_butter_layers.json` 默认实例与 `-NoInstance` 各 173 条命令通过；冰像处刑者专项在两条路径各 185 条命令通过，并在同步截图中同时显示保持黄色的 `butter_splat` 与继承蓝色减速覆盖的 88% 黑盔。警铃草、急救员和绝缘甲 follower 父回归也在两条路径全部通过。
- 2026-08-27 修复 follower 漏提交 additive glow：冰像黑盔在减速覆盖下承伤时与父头轨同步闪白，同轨黄油仍保持黄色且不发光。`smoke_ice_statue_executioner.json` 扩为 196 条命令，在默认 Vulkan、`-NoInstance` Vulkan 和强制 OpenGL 三条当前桌面可见路径均 exit 0、`status=passed`，三条路径截图均确认黑盔发光。
