...
- `clang-release` 构建通过。主人当前桌面可见运行 `smoke_ladder_zombie.json`，窗口标题“植物大战僵尸中文版”、exit 0、208 条命令、`run.log` 为 `script finished OK`；除携梯、放梯、共享攀爬、死亡与磁吸外，草坪双坚果路径断言第二目标正常开吃，屋顶专项则在旧梯子宿主与相邻空花盆碰撞重叠期内断言立即进入 `anim_eat`、实际扣血，锁定不再平移。主人提供的 `level37_data.json` 只读复现也从修复前约 267 帧平移收敛为加载后第 1 个逻辑帧开吃、X 仅移动 0.12。现有 `smoke_polevaulter_vault_walk.json`、`smoke_magnetshroom.json`、`smoke_jalapeno.json` 同样可见 exit 0。
