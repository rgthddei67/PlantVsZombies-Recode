...
`GetNormalPlantAt` 等常用棋盘门面没有迁移。当前 `Board.cpp` 从 8610 行降至 6073 行，新增天气文件
2592 行。`clang-release` 完整构建、Win7 378 项导入审计与 CTest 3/3 通过；桌面可见普通雨链、
预报、台风、4-9 暴风雨夜和督军改天专项均通过，关键黑屏、白闪和阵风截图已目验。
