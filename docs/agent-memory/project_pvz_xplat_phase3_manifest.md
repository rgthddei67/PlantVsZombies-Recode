...
2026-07-22 运行资产收敛为 `build/clang-release/resources` 单份实体；其他 preset 的同名路径是 NTFS Junction。清单脚本仍从 exe 旁的 `./resources` 枚举，因此无需修改运行时契约，各 preset 构建时会对同一份权威资源重生成同一清单。
