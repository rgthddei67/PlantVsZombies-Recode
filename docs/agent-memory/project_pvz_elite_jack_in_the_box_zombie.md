...
- 每次随机等待 5～7 游戏秒后只投 1 个盒子；冻结、无头或死亡动画阻止开始新投掷，
  ...
  当前实体、实际卡槽/冷却和正式合法格，再由纯数值模块以 64 rollout、16 秒时域、
...
- 2026-08-02 主人指定精英小丑免疫磁力菇，`HasMagneticItem()` 固定 false；磁力菇在
  近距离仍保持 READY，盒子/把手、RUNNING 阶段与投掷能力不被废除。派生存档现先调用
  普通小丑基类，确保共享 phase/循环声状态不会因派生覆写而丢失。
...
- 2026-08-14 植物选点预算与急救员选疗拆分后，精英小丑改为 64 rollout，16 秒时域和
  16 只详细样本上限不变；`clang-release`、378 项 Win7 导入审计与三项 CTest 通过。
  本次数值调整按主人要求未运行 AutoTest，相关脚本预期已同步到 64。
...
- `smoke_fog_spawnlists_4_1_to_4_6`、`smoke_elite_jack_almanac`、普通小丑行为和
  ...
  紫色双臂和紫金盒子正常。
- 2026-08-04 起，未魅惑投盒的实际命中植物会选择自身九宫格内最近的南瓜；同一次
  爆炸按保护者 ID 归并，每枚南瓜只承受一次 `50×5=250`。无保护者仍对每个命中层扣 50；
  贪心评分和蒙特卡洛快照同步使用九宫格、稳定并列与保护者归并语义。
