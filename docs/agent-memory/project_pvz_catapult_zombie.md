...
投篮状态机为 `WALKING/SHOOTING/RELOADING/CALTROP_DYING`。初始 12 球，`anim_shoot` 第 46 帧发射 75 伤篮球，固定飞行 1.2 秒、拱高 210px，片段结束扣库存；篮筐继续只显示既有四球，库存大于等于 5 时全显，进入 4/3/2/1 后逐个减少。有球时装填 3 秒，无球后永久前进。篮球走已有 Bullet 对象池但只碰植物，按同格 overlay/normal/pumpkin/under 重新解析目标层。`Bullet::HandlePlantContact` 的实际扣血前保留 `TODO(叶子保护伞)`，未来拦截只接管这一入口。
...
既有验证证据：六发版本的可见 `smoke_catapult_zombie.json` 118 条命令 exit 0，覆盖第 46 帧前后、75 伤、耗尽、装填快照、碾压、两段损坏、不可魅惑、普通爆炸、化灰第 29 帧、爆胎延迟爆炸及爆胎阶段停止碾压，9 张同步截图已检查。共享回归 `smoke_caltrop.json`、`smoke_cabbagepult.json`、`smoke_kernelpult.json` 均可见 exit 0。2026-08-08 主人把逻辑库存调为 12；专项期望已同步为初始 12、首发后 11、十二发共 900 伤与耗尽，但按主人要求本次不运行 AutoTest。最终只执行静态检查和 `clang-release` 配置/编译，exit 0；链接仍输出既有 vcpkg applocal 找不到 objdump 的非阻断警告。
