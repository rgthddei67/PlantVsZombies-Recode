...
- `ZOMBIE_POGO` 由独立 `PogoZombie` 实现，500 本体生命；生存权重 1000，第 10 波、第 10 轮起可选。当前屋顶冒险关尚未接入，因此没有把它硬塞进非屋顶关卡。它没有水路状态机，`CanZombieTypeSpawnInPool` 必须返回 false，使泳池正式选行只落在四条陆路。
...
- `smoke_pogo_zombie.json` 覆盖资源、陆路/水路兼容性、弹跳/越障/高坚果、啃食、断肢掉头、存档和地面植物交互。1.6 倍下落与 0.75 粒子的上一轮脚本运行退出码为 0，并由主人可见实机截图校准；最终整段空中运动不受寒冰拖慢、下落 2.0 倍与禁刷水路的调整按主人要求未重跑 AutoTest，只完成 `clang-release` 编译与静态检查。
