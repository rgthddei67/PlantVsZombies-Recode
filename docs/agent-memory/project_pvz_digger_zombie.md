...
- 七阶段状态机为地下、持镐出土、眩晕、持镐折返、地下丢镐停顿、无镐出土、
  ...
  镜像和失败线判定，避免只翻视觉不改规则。
- 普通与精英矿工均只允许正式波次刷新到陆地行；泳池背景的第 2/3 行水路由
  `Board::CanZombieTypeSpawnInPool` 统一排除，陆地行仍参与原有平滑加权选行。
...
- 2026-08-02 普通矿工在地下、眩晕或持镐行走期可被磁力菇吸镐，剥离复用 `LosePickaxe()`
  进入既有无镐状态机；地下碰撞体虽关闭，磁力搜索仍以其包围盒量距。主人指定爆破工头
  `HasMagneticItem()` 固定 false，磁力菇不能取消其爆破预警与核心能力。
...
- 2026-08-04 爆破仍使用房屋侧三列×相邻三行格区；每个命中层改由自身九宫格内最近
  南瓜保护，同一南瓜无论覆盖多少命中格只承受一次 `150×4=600`。没有九宫格保护者的
  实际命中层仍各扣 150，南瓜之间不连锁。
- `smoke_digger_spawn_rows` 在泳池背景逐项确认普通与精英矿工的陆路行 0/5 可刷新、
  水路行 2/3 不可刷新，并保留可见截图、状态 JSON 与 `run.log`。
