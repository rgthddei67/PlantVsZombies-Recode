...
- 后续 21227e2：spawnlists.json 补全黑夜 10~18（**9关一大关，2-9=level 18 才是黑夜收官**，19 已是 3-1）；18=全阵容30波+sun字段初始阳光1500（LoadSpawnListFromJson 新增可选 sun，读档由存档覆盖）；dump_state 出怪池不再限生存模式；spawnlists.json 照 gamedata 先例 force-add 进 git。
