...
- **巨人锤击**（2026-08-15）：清醒寒冰菇覆写 `ResolveGargantuarSmash()`，直接复用自然第16帧的完整冻结入口，立即播放冻结音效/白闪、逐行 `StartFrozen()` 并死亡，不等待剩余动画；睡眠寒冰菇仍回退通用压扁。行为对齐 C# `Plant::Squish()` 对未睡眠 `Iceshroom` 直接 `DoSpecial()` 的分支，巨人侧不增加植物类型判断。
...
- 2026-08-15 巨人锤击分支完成 `clang-release` 编译与 Win7 导入审计；主人明确要求不跑 AutoTest，因此本条没有新增运行时证据。
