...
- 作弊守卫在收费点：`Card::UpdateCooldown/StartCooldown`（无冷却）、`CardSlotManager::CanAfford/SpendSun`（无视阳光），全部 `GameAPP::mDevelopMode && mDevXxx` 双条件，无 -develop 零行为变化。
...
15e109d 补两修：①旧 `CardDisplayComponent::UpdateCardState` 曾裸比阳光绕过 CanAfford→作弊开了卡不亮；该显示状态现由 `Card::UpdateVisualState` 直接调用 manager 的收费资格（收费点守卫要连显示层一起查）；②下一波改直调新提取的 `Board::SummonNextWave()`（原 Update 出波序列提取成公有方法），按钮 autoClose=false 暂停中连点连出。
