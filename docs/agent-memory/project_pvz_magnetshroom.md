...
- 2026-08-12 通用 `MagneticItem` 增加默认 0 的 `extractorSelfDamage`。绝缘僵尸一次原子卸下整件
  陶瓷胸甲并返回 150；磁力菇先进入吸取/充能、接管离体物，再直接扣自身本体生命，绕过南瓜和
  防御词条。第二次吸取可令 300 生命磁力菇死亡，但目标胸甲不会回滚。

## 视觉、资源与存档

- 离体装备从当前装备轨道世界位置起飞，每固定逻辑步移动剩余向量的 5%。C# 的
  `MagnetItem.mDestOffset` 是未缩放贴图左上角，本项目 `worldPosition` 和 `DrawTexture`
  使用贴图中心；目标点必须以植物动态视觉锚点为基准，再加该装备缩放后的半宽、半高。
  初版统一 `+25px X` 只碰巧缓解横向偏左，却让所有装备继续上移半张贴图，铁门最明显；
  禁止用统一 X/Y 补偿代替逐贴图锚点换算。贴图使用装备当前损伤阶段，抵达后保留到
  15 秒充能结束。
...
- 同日主人实际游玩截图发现此前装备终点整体过高，铁门最明显；根因是把 C# 左上角偏移
  当成 C++ 绘制中心。改为逐贴图半尺寸换算后，2026-08-12 当前桌面可见完整
  `smoke_magnetshroom` 已 exit 0，并在绝缘僵尸专项中继续验证离体胸甲与吸取存档。
