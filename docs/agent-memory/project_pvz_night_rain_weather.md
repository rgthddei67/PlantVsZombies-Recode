...
天气预警：每个晴/雨阶段结束前 15 秒预抽取并锁定真实下一天气；公开预报有 75% 概率显示真实结果，另外 25% 必定显示其余三种天气之一。左上角面板用未缩放时间滑入/滑出，第一行显示当前天气，第二行显示公开预报与揭晓倒计时；天气揭晓后继续显示当前结果 5 秒，随后即使仍在下雨也会收回，直到下一次预警再出现，避免长期遮挡视野。揭晓不一致时在面板下方弹出约 3.2 秒的非模态红色提示，显示“预报 → 实际”，不暂停也不抢输入。公开预报、真实下一天气、ready 标记和当前天气展板的剩余显示秒数均进关卡存档：显示 4 秒后保存，读档只续 1 秒；已经归零并消失的展板不会因仍在下雨而重现。失败提示的剩余时间、错误预报与实际天气也一起保存，读档从原动画进度继续，归零后不重播；只有主面板的滑动进度不存档。
