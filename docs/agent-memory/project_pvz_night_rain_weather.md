...
- `smoke_night_rain.json` 用 `set_weather`/`advance_weather_phase`/`trigger_lightning` 固定状态，断言三档倍率、暗幕、雨声启停、词条组合、减速、冻结优先级、大雨闪电及“小→大→中→小→晴”完整链，并产出六张截图；另用 0.2 秒真实天气倒计时验证尾端小雨自动复位倍率并停止声音。
- 2026-07-20 `clang-release` 零警告；可见 `smoke_night_rain` 69 命令通过；既有白天 `smoke_perks_attackspeed` 通过且天气倍率保持 1.0。
