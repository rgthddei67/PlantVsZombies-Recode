...
`baseHealth`、`attackDps`、`attackRowRadius`、`sunPerSecond`、`firstSunDelay`、
`persistent` 和 `supportOnly`。现有类型已全部显式填写；未来新增植物由 adding-plant
流程要求补齐并用 `plantDefinitions.*.simulation*` 投影验证。复杂一次性能力填
`persistent:false`，普通花盆/睡莲填 `supportOnly:true` 后进入独立每格支撑数组且不作为
未来种植卡；避雷花盆等带特殊推演能力的 under 植物保持 false。不得把植物类型判断堆进
Board。该对象保持可选，以兼容旧资源和未参与推演的扩展条目。
