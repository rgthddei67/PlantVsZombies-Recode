...
2026-08-02 磁力菇使用独立装备剥离路径：立即清除粉色头盔并吸走当前损伤阶段贴图，但不调用派生 `HelmDrop()`，因此不发射 `ZombiePinkFootballOff`，也不结算半径 120 的掉盔范围伤害；专项锁定磁力菇自身生命仍为 300。

2026-08-04 南瓜范围拦截扩大为逻辑九宫格：圆内每个实际命中层选择最近活动南瓜，同一爆炸按保护者 ID 归并一次 `50×5=250`；水池睡莲与普通层可由相邻行南瓜保护，无保护者仍保持每株 50，普通小丑不接入。
