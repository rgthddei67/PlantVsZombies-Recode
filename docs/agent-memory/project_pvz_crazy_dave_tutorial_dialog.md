...
模态闲聊。当前覆盖 2-1、4-1、4-2、4-9、5-1、6-1、6-9、7-1、7-8 和 7-9；按主人要求不在
1-1、3-1出现，也不逐个解释僵尸。文字只借天气、迷雾、路灯花燃料、屋顶坡面与径流、
黑夜屋顶雷荷、动态雾和寒潮剧情给出含蓄提示。7-8/7-9 的三句分别明确开局不久会有一阵强寒潮、
待其回暖后天气恢复既有随机节奏；对话停留、选卡和暂停均不消耗 Board 开战后的 12 秒倒计时。
...
- 最终 `clang-release` 全量构建、Win7 378 项导入审计和 CTest 3/3 通过；当前桌面可见
  ...
  透明身体、中文台词和对话框无视觉回归。
- 2026-08-26 新增 7-8/7-9 后，`clang-release` 默认 Vulkan 与 `-NoInstance` 当前桌面可见
  `smoke_crazy_dave_dialog.json` 都执行 152 条命令到 command 151、exit 0、`status=passed`；
  两条新增闲聊各为三句，最终已读集合为 10 个关卡号、语音播放请求为 31。截图目验两关的戴夫
  透明身体、中文台词、气泡和页码布局正常，`-NoInstance` 的活动状态明确导出
  `usedInstancePath=false`。
