...
description: 继承式玩法对象正式架构；Card、CardSlotManager、显式 Transform 与纯 UI 所有权已完成，后续迁移 Collider/Shadow/Clickable
metadata:
  node_type: memory
  type: project
...
3. Collider 下一阶段改为宿主显式拥有并原子注册/注销。
4. Shadow 与 Clickable 保留可选组合能力，但脱离通用 Component 生命周期。
