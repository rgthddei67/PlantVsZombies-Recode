...
description: 继承式玩法对象正式架构；Card、CardSlotManager、显式 Transform、纯 UI、Collider 与 Shadow 所有权已完成，后续迁移 Clickable
metadata:
  node_type: memory
  type: project
...
当前 `GameObject` 的 `unordered_map<type_index, unique_ptr<Component>>` 是早期框架遗留的横切附件容器，不再作为新玩法系统的默认扩展点。2026-08-16 已把 `CardComponent` 与 `CardDisplayComponent` 的状态、主线程缓存和绘制职责直接并入 `Card`，并把 `CardSlotManager` 从组件改为 `GameScene` 独占的普通控制器；存档 JSON 字段不变。2026-08-22 又依次把 Transform 改为 `GameObject` 内按需创建的 `std::optional<Transform>`，把 Collider 与 Shadow 改为 `GameObject` 的显式可选所有权并脱离 Component 生命周期。容器当前只承载 Clickable，完成其独立迁移后即可删除通用 Component 框架。
...
4. Shadow 已改为显式可选附件；下一阶段独立迁移 Clickable，保留稀疏自注册能力但脱离通用 Component 生命周期。
