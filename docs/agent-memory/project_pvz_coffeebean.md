...
description: 经典咖啡豆、短时 flying overlay 层、蘑菇唤醒状态与毁灭菇激活的当前契约
metadata:
  node_type: memory
  type: project
---
...
- 当前桌面可见 `smoke_coffeebean.json` 默认与 `-NoInstance` 各 102 条命令退出 0，`run.log` 均以 `script finished OK` 结束。覆盖资源与数值、非法落点、等待/碎裂快照、音效不重播、唤醒弹性、普通小喷菇、日间泳池毁灭菇、水上弹坑与台风全层搬运；7 张阶段截图均已目验。
