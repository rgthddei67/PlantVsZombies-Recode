...
- [第八大关机枪射手与极光树桩](project_pvz_gatling_aurora_torchwood.md) — 2026-08-29 当前实现仍为 8-2 机枪射手、8-3 极光树桩；后续植物设计已批准在北极星花/冰镜草实现时重排为 8-3 极光树桩、8-4 机枪射手。两者均为225/30/300紫卡，四连发与50伤四目标极光弹玩法不变
- [第八大关热感狙击僵尸](project_pvz_thermal_sniper_zombie.md) — 2026-08-29 设计定案、尚未实现：800本体/0.8倍移速/50啃食；玩家种植时同排全部就绪个体各自0.3秒瞄准并发射基础生命等伤害的双向热脉冲，沿途植物逐发拦截、原位置终止、独立3秒装填；8-5每波限3、8-6每波限4，均无同时或累计上限
- [第八大关北极星花与冰镜草](project_pvz_polar_night_support_plants.md) — 2026-08-29 设计定案、尚未实现：奖励拟重排为8-2北极星花、8-3极光树桩、8-4机枪、8-5冰镜草、8-6无奖励；北极星花125/30/300，以12秒蓄能换8秒3x3按需导航；冰镜草100/20/300，每12秒逐面凝结且最多两面，一镜抵消一枚穿格敌方水平直射弹
...
- [黑夜屋顶雷荷与基础放电](project_pvz_night_roof_charge.md) — 2026-08-22 仅 `NIGHT_ROOF` 启用独立雷荷；晴/小/中/大雨基础为-0.5/1/2/3点每秒，场上有效劫持者令雨中固定再+4.1且多只不叠加；满100锁行预警4秒再放电0.65秒，活动期正向输入截留为最多15%余电；普通/湿坡放电与接地菇契约保持
...
- [架构边界、存档版本与技能审计门禁](project_pvz_architecture_boundaries.md) — 2026-08-26 `BoardPresentation` 取代 `Board::mGameScene`，Board 保持天气/波次/生存玩法唯一权威；`SceneManager` 明确单活动场景；玩家 schema v2-v5 与关卡 schema v7（含冒险红眼每波计数和开幕寒潮标志）迁移保持事务式并有纯测试；每次任务提交前审计相关 skills
...
- [九关制冒险进度+显式植物奖励表](project_pvz_adventure_progression.md) — 2026-08-26 `AdventureProgression.h` 已覆盖七大关63小关；5-9奖励接地菇，第六大关6-2～6-8依次完成忧郁菇、双子向日葵、避雷花盆、冰瓜、玉米加农炮、磁暴菇和模仿者，6-1/6-9无奖励；冬日花园7-1雪锚果、7-2融雪投手、7-4伏霜雷、7-6警铃草、7-7炉芯花均已登记，7-3/7-5/7-8/7-9无奖励
...
- [蹦极僵尸](project_pvz_bungee_zombie.md) — 2026-08-22 四阶段下降/抓取/快速上升状态机；MC开启以48 rollout按单株实体移除并最多推进16只，关闭复刻10000:1网格加权随机；普通层明确拒选时遮蔽下方承载层，因此双格玉米炮下两只花盆不会成为候选；巨人砸击未改；植物双向关系与身体后层/植物/前臂绘制入档，可见专项通过
