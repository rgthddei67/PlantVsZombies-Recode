...
- [第七大关冰像处刑者](project_pvz_ice_statue_executioner_zombie.md) — 2026-08-26 `ZOMBIE_ICE_STATUE_EXECUTIONER` 为300本体＋2700黑色橄榄头盔，在冻土锁定高价值持久植物并以三次2.5秒前摇、每击40伤完成处决；炉芯花可在封存前硬阻断并耗尽其一次性能力；灰烬按本体＋当前防具总耐久判断直消；88%静态头盔、同轨命名vector follower、破帽粒子、存档、每波限五及专项闭环
...
- [经典玉米加农炮与双格植物占用](project_pvz_cob_cannon.md) — 2026-08-25 `PLANT_COBCANNON` 为6-6奖励：双玉米投手升级为一个ID别名占两格的单实体；READY炮弹轨按原版0.75秒明暗闪烁；第78帧发射2.0秒定点炮弹，飞行只画横向3倍的普通 `PLANTSHADOW` 并随垂降从约0.5倍长回1倍，不保留固定黑色落点标记；115px半径三行1800灰烬伤害、落地焦痕、飞行气球资格、爆心格3×3清梯及双格危险契约保持
