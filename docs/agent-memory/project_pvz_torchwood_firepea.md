...
2026-08-02 毒豆纳入同一转换链：`BULLET_TOXICPEA` 穿过火炬后变为独立 `BULLET_TOXICFIREBALL`，享受普通火豆的 40 伤、解冻、抗火和溅射伤害语义，并使用紫焰飞行/命中特效；紫焰溅射宽度独立收窄为沿飞行方向 30 像素，直击与实际受溅射目标均附毒。当前类型明确入档，不可变 `mPoolType` 仍保留 ToxicPea 回收归属，复用后恢复 15 伤纯紫毒豆。

最终 `clang-release` 构建通过；桌面可见 `smoke_torchwood.json` 退出 0、25 条状态断言全绿，确认普通豌豆仍转换为 `BULLET_FIREBALL` 且紫焰计数为 0。毒囊专项默认与 `-NoInstance` 均退出 0，独立紫焰类型、存读档、直击附毒和对象池复位通过；紫焰 30 像素溅射附毒的后续回归证据见毒囊射手主题。

2026-08-02 紫焰范围独立调为 30 像素后再次运行桌面可见 `smoke_torchwood.json`：退出 0、25 条状态断言全绿，普通火豆双目标总生命仍为 487，确认其 100 像素溅射未被紫焰分支改动。
