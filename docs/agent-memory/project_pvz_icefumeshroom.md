...
description: 寒冰大喷菇(自创变种)a70506b+削弱de5e528均commit未push；数值主人已裁定中档(2.5s间隔/2.0s减速/150阳光)；AOE减速=乘法光环的平衡论证；旧Trophy枚举解锁耦合已由AdventureProgression表取代
metadata:
  node_type: memory
  type: project
...
**历史发现（已于 2026-07-18 被显式表取代）：** 当时 `Trophy.cpp` 用
`static_cast<PlantType>(mBoard->mLevel)` 解锁卡，曾迫使 PlantType 枚举顺序等于关卡奖励顺序。
现由 `AdventureProgression.h::PLANT_REWARD_BY_LEVEL` 显式映射，新增/调整奖励不再插枚举占位；
但已实现植物的枚举整数仍不能改动，否则旧存档 `havecards` 会错位。
