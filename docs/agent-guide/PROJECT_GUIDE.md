...
- **轻量蒙特卡洛 AI 测试：** `set_monte_carlo_ai` 用 `value=true/false` 切换 `GameAPP::mEnableMonteCarloAI`；`reset_test_state` 和带复位的 `goto_level` 都恢复为默认开启。状态 JSON 的 `monteCarloAIEnabled` 锁定总开关，精英小丑另导出 `eliteJackTargetingMode` 及 rollout/候选/僵尸/卡槽计数，用于同时证明蒙特卡洛路径和低配贪心回退。
