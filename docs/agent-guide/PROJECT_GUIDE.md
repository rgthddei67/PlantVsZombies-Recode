...
- **迷雾测试命令：** `set_fog_weather` 用 `intensity=CLEAR/DENSE` 与 `duration` 固定四大关雾势；`set_fog_forecast` 用 `forecast/actual=CLEAR/DENSE` 和 `revealIn` 固定独立雾势预报；`set_fog_dispersal` 用 `value=0..1` 固定台风驱散进度。`dump_state.fog` 导出基础/有效左边界、逐列最大 alpha、可见雾格数、驱散百分比、有符号风向位移、预报、贴图分片加载数与动态大雾概率。迷雾脚本必须同时覆盖非四大关 no-op、小雾/大雾覆盖距离、双向风偏移、完全吹散、停风回流、预报和关卡快照往返。
