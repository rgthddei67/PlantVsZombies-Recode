...
- **产物：** 位于 `build\<preset>\autotest\out\<script-name>\`，包括 PNG、`state.json` 和 `run.log`。Release 会裁掉 Logger INFO，因此 `run.log` 是权威命令执行记录；首帧会额外记录实际 Vulkan API、dynamic rendering 路径与同步路径，防止兼容测试只传了参数却未命中目标分支。
...
| `Graphics` | `Graphics.cpp` | 自定义 Vulkan 1.2+ 封装，含变换栈与批渲染；1.3/KHR/传统路径按能力选择，SDL2 提供 loader/Surface，Volk提供函数分发 |
