...
- **地图覆盖夹具：** `goto_level` 可选 `background=GROUND_DAY/GROUND_NIGHT/WATER_POOL/NIGHT_WATER_POOL/ROOF/NIGHT_ROOF`，只在 AutoTest 场景创建时覆盖正式关卡背景；字段缺省时始终使用 `GameAPP::GetBackgroundID`。当前 5-9 已正式改为白天 `ROOF`，夜屋顶专项用显式 `NIGHT_ROOF` 覆盖继续验证未来第六大关资源与机制，禁止把该夹具接入正常玩家入口。
