...
- **黑夜屋顶雷荷命令：** `set_night_roof_charge` 仅对 `NIGHT_ROOF` 生效，用 `phase=CHARGING/WARNING/DISCHARGING`、`charge`、活动阶段的 `row` 和可选 `remaining` 固定雷荷状态；`dump_state.weather.nightRoofCharge` 导出资格、积累百分比、阶段、锁定行、剩余毫秒和放电进度。
