...
- **Command set:** `goto_level` / `choose_cards` / `wait_state` / `set_sun` / `plant` / `spawn_zombie` / `wait_seconds` / `wait_frames` / `set_timescale` / `charm_zombie` / `click` / `key` / `screenshot` / `dump_state` / `quit`. Wait-type commands accept `timeout` (default 15s). Plant/zombie types use the enum identifier verbatim (e.g. `PLANT_PEASHOOTER`, `ZOMBIE_FASTPAPER`); new types need a line added to the name table in `Game/AutoTest/TestDriver.cpp`.
