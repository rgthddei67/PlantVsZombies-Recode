...
- 主人长期明确授权：Codex 完成本仓库任务并验证通过后，默认提交并 push，无需主人逐次回复“push”。授权目的地为 `origin`：`https://github.com/rgthddei67/PlantVsZombies-Recode.git`，推送到当前工作分支已明确配置的上游分支（当前为 `master` → `origin/master`）；授权包含任务相关源码、资源、文档及对应提交历史的上传与发布。
