...
**所有玩家可感知的歧义或分支都必须逐项问主人。** 包括但不限于例外对象、来源归属、基础/紫卡/运行时变种是否视为同类、伤害与附带状态是否一起豁免、范围/溅射口径、提交边沿、存读档终态和关卡投放。主人批准一个具体例子只批准该例，禁止按常识、实现便利或相邻品种自行外推；先提出当前最小决策问题并记录明确答案，未定分支保持未实现或中性，不能边做边替主人补规格。
...
- **弹丸来源身份在发射边沿锁定**：需要按植物身份结算免疫、适应、统计或其他目标效果时，弹丸必须显式携带原发射植物的稳定语义身份，不能从共享 `BulletType` 反推。火炬树桩等运行时转换只改变当前弹型、伤害、状态和表现，不把来源转移给转换植物；该身份必须随在途存档恢复，并在对象池 `Reset()` 归零后由下一位发射者重新设置。基础植物与紫卡升级是否归为同一谱系、灰烬等跨植物类别如何归并，均按主人已定规格显式规范化；出现未覆盖归属时先问主人，禁止自行扩类。
