...
25. **天气条件式一次防御只保存消费结果，不复制天气资格**：植物从 `Board` 当前真实格况（如 `IsCellFrozen`）派生是否可用，并通过通用语义接口原子返回拦截、散射收束或后续伤害倍率；来袭僵尸/环境威胁继续拥有自身基础伤害、动作时序、音画与乘员落点，禁止按植物类型分支。倍率保持浮点语义并由威胁在逐格伤害入口统一取整，植物不得复制基础伤害或提前把分数换成整数伤害；专项同时断言倍率整数投影与后续目标的实际扣血。若每实体一生只能触发一次，只保存 `spent`，解冻/再冻结不得返还；加载用中性默认兼容旧档并静默重建当前材质。专项覆盖未满足天气、满足天气、首次消费、重复请求、解冻/再冻结、快照往返、基类中性响应和条件外观截图。
