...
9. **整株世界变换必须覆盖复合 Animator 的两条绘制路径**：带附件的根 Animator（如射手身体）会回退慢路径，附件自身若无子级仍会递归进入 GPU 实例化快路径；外层 `Graphics` 变换栈只覆盖慢路径，会出现“身体变、头部不变”。应在 `Animator` 最终矩阵/`InstanceRecord` 两处统一实现并递归同步现有与以后附加的子 Animator；AutoTest 同时断言根/子变换并逐张检查截图。
