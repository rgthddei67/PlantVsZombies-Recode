...
1. 普通植物功能迭代先用 `msvc-debug` 缩短增量构建并保留 Debug CRT/Debug 语义，功能完成后必须整体配置、编译 `clang-release`，再用该产物完成最终相关回归；性能、内存布局、并发、LTO 或 Release-only 行为从一开始就全程使用 `clang-release`。只有明确需要 Clang 且无 LTO 的诊断才使用 `clang-playtest`，诊断结果不能替代最终 Release 证据。
