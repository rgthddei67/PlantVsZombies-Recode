...
- 地图专属天气调色若只应改变静态背景，必须注册在 `LAYER_BACKGROUND` 与战场实体之间并消费平滑后的天气 getter；禁止塞进全世界覆盖层而重复染色实体。若用全尺寸变体贴图交叉淡化，必须由原背景按蒙版合成，保证尺寸、网格与非天气区域像素对齐，并补齐“文件/清单 → loader 注册 → 资源键 → AutoTest 加载断言”。白天屋顶雨景只对 `ROOF` 生效，`NIGHT_ROOF` 保持 no-op。
