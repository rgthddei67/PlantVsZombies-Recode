...
- 植物阵风位移继续由 `Board` 的唯一逐格结算点负责；抗风植物用虚接口声明“锚定格位”和直接撞击反馈，禁止按 `PlantType` 写死分支。源格含锚定植物时整个上下层组合不动；只有来袭格直接进入锚定目标格才结算，普通占格形成的后方链不传压。伤害逐格立即生效以允许中途死亡后放行剩余步数，同阵风音画可按锚定植物合并；这类纯派生能力不新增存档字段，但要导出直接阻挡格次供测试。
