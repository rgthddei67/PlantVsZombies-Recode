...
- 积累器的一次性放电效果只接在 `WARNING -> DISCHARGING` 等唯一阶段边沿：先复制稳定实体 ID，再按当帧连续环境状态分区，最后调用植物/僵尸通用状态接口。实体生命与状态余时自行入档；恢复已经处于 `DISCHARGING` 的 Board 只恢复演出阶段，绝不重新结算。连续径流等区域场保留 Board 查询，不能伪装成任意固定秒数；实体的离散停机与连续区域源应由统一有效查询合并。
