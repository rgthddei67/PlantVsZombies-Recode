...
本文件记录截至 2026-08-22 的当前实现。动手前用文中的搜索词核实源码；当前代码优先于本文件。
...
下一段真实天气为大雨时，Board 仍在预报锁定期预抽完整待生效台风状态；但提前 5 游戏秒的
大雨分级文字警报只在公开预报与真实下一天气相同（均为 `HEAVY`）时显示。误报不得借该警报
提前揭晓真实天气；隐藏警报也不得取消待生效台风状态，切档时仍消费同一份锁定结果。
