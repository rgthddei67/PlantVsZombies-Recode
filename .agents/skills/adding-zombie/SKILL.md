...
- **新增完全定身状态要审计全部派生 `Update()`**：基类在 `Zombie::Update()` 内早退只能停住基类移动/啃食，派生类在调用返回后仍可能继续倒计时、投掷或开盒。把冻结与新定身统一收敛到 `IsImmobilized()`，grep 所有覆写 `Update()` 的品种并在尚未离手的能力门禁前复用它；已经独立离手的攻击应在门禁前继续推进。专项至少锁定一个带派生倒计时的可定身品种，比较状态生效前后计时不再减少；免疫阶段则保持原行为。
