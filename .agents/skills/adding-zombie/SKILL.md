...
- **正向修复防具时破损外观必须可逆**：治疗只能增加当前仍大于 0 的生命池，不能重建已掉落、被磁吸或已部署的装备；结算后通过基类语义入口调用虚 `CheckHelmImage` / `CheckShieldImage`。各防具检查函数必须完全由当前耐久重新派生完整/轻损/重损阶段和贴图，不能保留只允许向下破损的一次性分支。专项同时断言耐久增加、跨阈值后的完整贴图，以及本体/一类/二类三层各自夹紧上限。
- **同一语义轨道并存多种状态挂件**：一个 `ReanimTrackExtra` 只有一个 follower 槽；黄油已占用时，第二种持续贴图不得覆盖它，也不得退回对象外 `DrawTexture`。为既有 PNG 建一帧小 reanim，首次施加状态时懒创建子 `Animator` 并 `AttachAnimator` 到已审计的语义轨道，强引用由僵尸持有、显隐只切子 Animator alpha；逻辑计时/倍率继续放宿主僵尸并入档，`FinalizeProtectedLoad()` 在派生外观恢复后按计时重建挂件，到期、濒死和 `Die()` 隐藏。资源闭环同时断言小 reanim 与实际 `IMAGE_REANIM_*` 键，默认实例化和 `-NoInstance` 各截一次最上排目标，防顶部 UI 裁切与双路径层级漂移。
...
- **高生命首领不能只提高 `mBodyHealth`**：逐项审计绕过数值耐久的统一入口。土豆雷会向 `TakePlantAshDamage(INT32_MAX)` 传直杀值，若需限制单次灰烬，应在该虚入口先压回基础上限再调用基类，以保留植物词条缩放；大嘴花走 `TakePlantInstantKill()`，拒绝吞噬时结算明确的替代伤害并返回 false。另按设计覆写 `CanBeCharmed()`、`ResistsTangleKelpDrowning()` 与 `CanBeKilledByMower()`；小推车免疫只跳过处决，车辆仍须正常启动、消耗并驶离。专项必须用真实植物/车辆覆盖每条入口，并以普通僵尸对照锁定默认直杀行为。
...
- **跳跃阻拦要拆分职责、动画时序和受伤对象**：阻拦植物通过虚接口声明“能挡哪类跳跃”并拥有 Bonk/粒子反馈；接触回调只锁定当前格顶层目标并起播跳跃，必须到 C#/规格给出的动画进度节点才查询一次，禁止一碰植物就提前阻拦。阻拦查询不能复用啃食顶层：须按格内层级逐层询问能力，让非阻拦南瓜继续向内找到高坚果；品种跳距超过基类时还要按起跳碰撞框到完整落地碰撞框的扫掠区，沿移动方向选择最先遇到的阻拦者，不能只复查起跳目标 ID，否则会越过后方高坚果。仅按进度检查不需要新增 `AddFrameEvent`；起跳 X、目标 ID、是否已检查和额外根位移须入档，读档恢复原动画帧继续判定，不能直接落地绕过阻拦。跳跃计时必须明确使用真实 delta 还是寒冰后的 scaled delta；若某半程按独立倍率加速，额外根位移必须消费同一时间基准和倍率，且不得把已缩放的 delta 再乘 Animator extra 倍率造成重复减速。被挡时先撤回品种已补的额外位移，再把碰撞框落到实际阻拦者迎敌面，恢复碰撞/阴影、弃杆或落地、切稳态并开始啃食，最后把实际阻拦植物传给派生钩子结算召唤或撞击。**规格写“给 A N 点伤害”先确认 A 是植物还是跳跃者，禁止把碰撞伤害凭感觉记到自身**；常规僵尸对植物伤害走 `Plant::TakeDamage(N, DamageSource::ZOMBIE)` 以统一消费僵尸增伤与植物韧性，只有主人明确要求最终固定扣血才另设不缩放入口。若要求被挡后仍召唤，先创建召唤物再伤害目标。AutoTest 必须同时断言节点前 `JUMPING/anim_jump`、无 Bonk/粒子/扣血，节点后才出现阻拦终态和派生效果，并覆盖“阻拦体外套非阻拦壳”与“长跳初始普通植物后还有阻拦体”。
...
- **投掷召唤物的离手事件必须先成功创建、后消费库存**：持有标志是背部/手中子单位显隐与能否再次投掷的唯一权威；创建失败保持库存和轨道，创建成功后再配置阵营、飞行参数与剩余减速，并最后清标志。巨人等投手保存动作 phase、是否已经越过离手帧、投掷距离和唯一持有标志；召唤物保存飞行/落地 phase、高度、水平/竖直速度与方向。读档分别覆盖离手前、离手后但动作未完和动作完成，禁止从现场是否存在召唤物反推库存。只抬高离手高度会延长重力飞行并带偏水平落点；若规格只要求视觉起点上抬，应先用旧高度和旧初速解出原落地时刻，再按新高度反算竖直初速，保持飞行总时长、水平速度和落点不变。若还要求水平距离乘倍率且水平速度不变，则把原落地时长乘同一倍率作为新目标时长，再结合新高度反算竖直初速，禁止同时盲乘高度、速度和距离。
- **不掉头的僵尸不要复用普通无头死亡阈值**：关闭 `mNeedDropHead/mNeedDropArm` 后，按品种规格在本体真正归零时启动死亡轨；生命仍大于 0 时继续完整行动。受伤换图若只由当前生命比例决定，读档直接重建材质，不额外保存易矛盾的伤势阶段字段。
...
- **可关闭的蒙特卡洛主动决策只放在动作边沿**：由 `Board` 一次采集实体、装备层、卡槽、已提交但未结算动作和确定的全局事件，纯数值模块用共同随机数比较全部合法候选与“不执行本次动作”基线，局部 RNG 禁止消费正式 `GameRandom`。详细实体数必须有共享硬上限；无上限范围效果把样本外对象折算成有界汇总量，不能改变正式结算数量。不同决策族可以共享实体硬上限，但 rollout 预算必须按各自时域、步数与候选规模使用独立可调常量；禁止为了调高短时域能力的精度而经同一个全局常量同步放大其他长时域推演。等待候选用有限延迟副本表达，累计等待和重试计时入档，达到上限后只比较立即动作；同收益严格偏好立即执行，等待必须靠真实未来收益胜出。总开关关闭、阵营模型不适用或推演失败时，原子回退该品种原有确定性/随机规则。AutoTest 同时导出决策模式、动作、rollout/候选/样本计数，覆盖开关两路、等待读档和共享上限；纯数值单测另锁定关键事件对收益的改变。
...
- **地图积累器联动**：同时使用 `adding-rain-weather`。若僵尸只在径流等满值锁定时影响目标行/路线，由 `Board` 在锁定瞬间通过语义窄的实体虚查询采样一次，按明确位置与 ID 规则选唯一候选；只替换已有目标而不扩大事件规模，随后只保存最终 mask/路线。WARNING 中死亡、后来出生和跨波残留不得重抽。若能力只强化该僵尸自身位移，在 `Zombie` 提供实例倍率虚入口并乘 Board 基础速度，禁止把加成施加给整行或复制天气状态到实体。AutoTest 固定种子断言替换前后 mask、候选死亡后不重抽，以及普通/精英同场速度投影。
...
- **全场稀有能力禁止靠每帧全表扫描维持锁定**：在 `EntityManager` 维护该品种的 `ID → weak_ptr` 稀疏索引，覆盖普通 Add、带 ID 读档重建和周期清理；只在能力阈值边沿遍历候选一次，Board 保存稳定实体 ID，后续锁定 Update/Draw 都做 O(1) 查询。若玩法另明确要求“场上存在任意该品种”持续改变积累速率，热路径也只能扫描这份稀疏索引并在首个有效候选处提前返回，禁止改扫全体僵尸或每帧创建候选向量。全场结算先快照全部目标再批量应用，先清除施法者锁定 ID，避免容器遍历顺序、死亡回调或同类连锁改变本帧结果。一次性锁定加当前/最大生命时保存 `boostApplied` 标志；Board 读档 finalize 重放锁定展示必须幂等，旧档缺标志可在这次 finalize 补应用一次，新档不得重复回血。AutoTest 同时覆盖并存择优/并列、持续增量多只不叠加、晴天归零、取消不转移、锁定生命存读档不重复和结算同帧快照。
