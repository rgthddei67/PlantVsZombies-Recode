...
- **一次性动作不能只靠完成分支解锁**：放梯、投掷、开盒等阶段若让 `ZombieMove` 早退，必须在断头、死亡、魅惑等使动作失效的边沿原子清理目标/计时并恢复可移动稳态；`LoadExtraData` 也要自愈旧档中的“终止状态 + 未完成动作”组合。否则品种更新会因终止状态早退，移动又因动作阶段早退，形成双重死锁。
