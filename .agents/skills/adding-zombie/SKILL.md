...
- **可复用的短视未来选点由 `Board` 采集、纯数值模块推演**：`Board` 是唯一读取 GameObject/当前卡槽/正式 `CanPlantAt` 的边界，快照记录当前敌方僵尸的真实行、X、移速、本体/头盔/护盾生命与攻击力，以及玩家实际已选卡、冷却、费用和 gamedata 植物画像；推演器不得按僵尸类型写 `switch`，只用这些能力维度固定预算推进。候选与无攻击基线共用 rollout seed，并使用局部 RNG，禁止消费 `GameRandom`。总开关放 `GameAPP` 且旧档默认开启，关闭或推演失败必须保留原确定性/贪心策略；AutoTest 同时锁定开启时的 rollout/候选/僵尸/卡槽数量和关闭时确实走回退。
