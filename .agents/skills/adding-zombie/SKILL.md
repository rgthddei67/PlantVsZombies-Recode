...
- **全场稀有能力禁止靠每帧全表扫描维持锁定**：在 `EntityManager` 维护该品种的 `ID → weak_ptr` 稀疏索引，覆盖普通 Add、带 ID 读档重建和周期清理；只在能力阈值边沿遍历候选一次，Board 保存稳定实体 ID，后续 Update/Draw 都做 O(1) 查询。全场结算先快照全部目标再批量应用，先清除施法者锁定 ID，避免容器遍历顺序、死亡回调或同类连锁改变本帧结果。AutoTest 同时覆盖并存择优/并列、取消不转移、读档锁定恢复和结算同帧快照。
