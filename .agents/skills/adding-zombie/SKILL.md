...
- 玩法对象架构固定为继承式：新增僵尸继续选择 `Zombie` 或最窄既有派生基类，并用窄虚接口表达品种差异；不得为僵尸能力新增玩法 `Component`、把品种状态拆进通用组件表，或为形式统一复制基类生命周期。空间数据由宿主 `CreateTransform()` 创建并通过 `GetTransform()` 访问，禁止重新引入 `TransformComponent`；其余横切附件在组件收缩完成前按当前源码接口使用。
...
- **投掷召唤物的离手事件必须先成功创建、后消费库存**：持有标志是背部/手中子单位显隐与能否再次投掷的唯一权威；创建失败保持库存和轨道，创建成功后再配置阵营、飞行参数与剩余减速，并最后清标志。巨人等投手保存动作 phase、是否已经越过离手帧、投掷距离和唯一持有标志；召唤物保存飞行/落地 phase、高度、水平/竖直速度与方向。读档分别覆盖离手前、离手后但动作未完和动作完成，禁止从现场是否存在召唤物反推库存。按世界 X 分界且只允许在一侧起手时，触发距离必须保留相对分界线的符号，禁止取绝对值后让单位越线越远反而重新满足条件；AutoTest 在分界线两侧各放一只半血静止投手，分别锁定允许起手与持续禁投。只抬高离手高度会延长重力飞行并带偏水平落点；若规格只要求视觉起点上抬，应先用旧高度和旧初速解出原落地时刻，再按新高度反算竖直初速，保持飞行总时长、水平速度和落点不变。若还要求水平距离乘倍率且水平速度不变，则把原落地时长乘同一倍率作为新目标时长，再结合新高度反算竖直初速，禁止同时盲乘高度、速度和距离。
