...
- **控制免疫使用基类能力维度，不按品种散落判断**：临时免疫通过 `ZombieControlEffect` 位、独立计时与 `GrantControlImmunity()` 发放；派生类的固有免控只覆写永久 mask。所有入口分别守卫 `SLOW/FROZEN/BUTTER/PARALYSIS`，发放时只清除 mask 覆盖的既有状态，计时按游戏时间推进并进入僵尸存档；装备触发后授予的免控属于本体，除非规格明确，否则装备掉落不得提前取消。寒冰菇的固定伤害、冻结和减速尾巴是三项独立语义：只免冻结时仍受伤且仍减速；黄油弹也应先结算命中伤害，再由 `ApplyButter()` 决定是否定身。`CanBeChilled()` 仍是减速状态、蓝色覆色和减速音效的共同前置总闸；寒冰子弹必须先检查它再播放 `SOUND_COOLDOWNZOMBIE`，避免“没减速却有音效”。蒙特卡洛快照必须同时带入临时余时和永久 mask 的有效结果。AutoTest 覆盖清除既有控制、未免疫类型保留、伤害与控制拆分、装备掉落、计时存读档和到期恢复，并用普通僵尸作对照。
