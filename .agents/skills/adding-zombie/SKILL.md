...
- **手动位移车辆的速度合成**：车辆若不用基类走路位移，仍必须让动画与真实 X 位移消费同一组能力、寒冰、雨势、台风倍率；禁止只改 `Animator` 速度。场地增益放大每个倍率时要分别围绕中性值 1 变换（如 `1.4→1.8`、`0.6→0.2`）再相乘，不能放大最终乘积。重叠场地允许叠层时，按仍存活且实际覆盖目标的独立来源逐层变换；进入、离开、来源死亡和范围缩短都要刷新，不能只缓存一个布尔值。
