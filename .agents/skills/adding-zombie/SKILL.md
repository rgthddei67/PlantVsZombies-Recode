...
- **跳跃阻拦要拆分职责和结算顺序**：阻拦植物通过虚接口声明“能挡哪类跳跃”并拥有 Bonk/粒子反馈，跳跃状态机只在唯一判定节点调用一次；品种额外代价放跳跃基类的派生钩子，先完成弃杆/落地和正常啃食状态，再最后结算可能致死的固定伤害，避免死亡动画被后续 `StartEat` 覆盖。规格写“固定损失 N 血”时不要走会消费免伤次数和僵尸减伤词条的 `TakeDamage`；无防具品种可直接走 `TakeBodyDamage`，并在 AutoTest 叠上两类词条证明仍扣足。若规格要求“即使被拦致死仍召唤”，必须先创建召唤物再扣伤害，并用后续逻辑帧断言 `anim_death`，不能在同一碰撞帧取证。
