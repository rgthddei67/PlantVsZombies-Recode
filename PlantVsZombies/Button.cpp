...
void Button::Draw(SDL_Renderer* renderer, const std::vector<SDL_Texture*>& textures) const
{
	...
	if (!text.empty())
	{
		...
		int textX = static_cast<int>(position.x + (size.x - text.length() * fontSize / 2) / 2);
		int textY = static_cast<int>(position.y + (size.y - fontSize) / 2);

		//RenderText(renderer, text, textX, textY, color);
