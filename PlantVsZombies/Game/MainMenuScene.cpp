...
void MainMenuScene::BuildDrawCommands()
{
	...
	RegisterDrawCommand("DrawLevel",
		[this](Graphics* g) {
			...
			// 坐标与冒险按钮 (558,85) 缩放 1.00 绑定：石碑贴图内角标的相对位置换算而来
			gameApp.DrawText(std::to_string(mBigLevel), Vector(708, 168),
				glm::vec4(255.0f, 255.0f, 255.0f, 255.0f));
			gameApp.DrawText(std::to_string(mSmallLevel), Vector(731, 170),
				glm::vec4(255.0f, 255.0f, 255.0f, 255.0f));
		},
		LAYER_UI + 10000);
