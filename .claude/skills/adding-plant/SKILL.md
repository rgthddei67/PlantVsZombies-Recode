---
name: adding-plant
description: Use when adding a new plant (新增植物) to PvZ, including plants whose effect touches zombies or other systems (冻结/减速/魅惑/穿透/新子弹, e.g. 寒冰菇 IceShroom) — proven on ScaredyShroom (胆小菇).
---

# 给 PvZ 新增植物

**最高原则：不确定的事立刻问主人，不要自行推断。** 已知必问项：①**帧事件的帧号**（主人会看动画预览，他有准确答案；从 reanim 活跃区间推断会错——胆小菇实测 25，区间末段推法给 28）②数值/行为想偏离原版时。
...
| 落点 | 先例 | 关键点 |
|---|---|---|
| **僵尸新状态效果**（冻结/减速/魅惑…） | 减速=SnowPea→`Zombie` `slowCooldown`；魅惑=HypnoShroom→`StartMindControlled` | 见下方专属清单 |
...
7. 动画冻结/变速类表现走 **Animator 三层速度模型**（`EffectiveSpeed=(clip?clip:base)*extra`，clip=0 回落 base，见 [[project_pvz_animator_clip_speed]]）；状态变色用 `OverrideColor`、特效用 `EmitEffect`。"全场"结算 = 对 0..4 行逐行 `ForEachZombieInRow`，没有整场 API。
7. **dump_state 加字段 + assert**（仿 `slowCooldown`/`armVisible`），否则 AutoTest 对新状态是瞎的。
